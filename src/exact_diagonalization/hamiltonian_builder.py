...
from src.exact_diagonalization.operators import count_pairs, flip_bit, check_if_bit_set, build_photon_number_matrix
    ...
    def build_hopping_matrix(self) -> csr_matrix:
        ...
        for k, state in enumerate(self.basis.fermion_states):
            for site in range(L - 1 + int(self.periodic)):
                ...
                next_site_occupied = check_if_bit_set(state, next_site)
                site_occupied = check_if_bit_set(state, site)
                if next_site_occupied and not site_occupied:
...
# Old implementation kept for reference without using Kronecker products and no Peierls phase
# class SlowHamiltonian:
#     """
#     Class to construct the Hamiltonian matrix for a given basis, hopping term t, and interaction term U.
#     Attributes:
#         basis (Basis): The basis object containing the states.
