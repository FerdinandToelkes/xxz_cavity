...
function ITensors.space(
    ...
    if occursin("Photon", string(S))
        ...
        error("SiteType not supported in this generic method")
    end
end
...
function ITensors.state(sn::StateName, st::SiteType{S}, s::Index; kwargs...) where {S}
    if occursin("Photon", string(S))
        ...
        return state(sn, st, s; kwargs...)
    end
end


function ITensors.op(on::OpName, st::SiteType{S}, ds::Int...; kwargs...) where {S}
    if occursin("Photon", string(S))
        return op(on, SiteType"Boson"(), ds...; kwargs...)
    else
        return op(on, st, ds...; kwargs...)
    end
end


function ITensors.op(on::OpName, st::SiteType{S}, s1::Index, s_tail::Index...; kwargs...) where {S}
    if occursin("Photon", string(S))
